...
### 1. `Dataset_Ejemplo/` (Muestras de Audio y Bitácora)
Contiene **4 archivos de audio (`.wav`)** y el archivo de registro **`Informe_Generacion_Dataset.tex`**. 
...
Adicionalmente, se incluye el archivo `.tex` mencionado, el cual es generado de forma automática por el script `Generar_Dataset.py`. Este documento contiene la bitácora matricial y paramétrica (valores de SNR, coeficientes $\alpha$ y $\beta$, tiempos de inicio) de los audios generados, sirviendo como respaldo matemático del proceso.
